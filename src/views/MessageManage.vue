...
export default {
  ...
  methods: {
    ...
    //获取用户收到的私信
    getMsgRec(uid, type, type) { //type=0,初始化时的调用 //type=0,初始化时的调用
      this.axios({
        method: 'get',
    ...
    //获取用户发送的私信
    getMsgSend(uid, type, type) { //type=0,初始化时的调用 //type=0,初始化时的调用
      this.axios({
        method: 'get',
  ...
  created() {
    this.getMsgRec(this.uid, 1);    this.getMsgRec(this.uid, 1);
  },
  mounted() {
    this.getMsgPlm(this.uid);
    console.log(this.msg_plm_has_new)
